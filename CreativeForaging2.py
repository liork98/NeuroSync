...
# get shape freqs
freq = pd.read_csv("gallery_freqs.csv")

# feedback points - initiate variables
points = [0]
POINTS = 0 
UPDATE_RATE = 30 # how often the originality score is updated 
show_points = visual.TextStim(win, text = "Your average originality score: 0", pos = [0.55, 0.2], color = "red", height = 0.02)
...
def get_freq(positions):
    
    try:
        fig = str(positions)
        idx = freq[freq['gallery_normalized']==fig].index.values.astype(int)[0]
        frequency = freq['shape_frequency'][idx]
...
def update_points(points, frequency):
    p = max(freq['shape_frequency']) - frequency
    points += [p]
    return points
    
def feedback(points, time):
    
    global POINTS
    
    # update every x secs 
    if time in range(0,EXP_TIME,UPDATE_RATE):
        if len(points) == 0:
            POINTS = 0
        elif len(points) > 0 and len(points) <= 3:
            POINTS = np.mean(points)
        else:
            POINTS = round(np.mean(points[-3:]),2)
    show_points.text = "Your average originality score: {}".format(POINTS) 
...
while not endTrial:
    ...
    for unit in units:
        ...
        while mouse1 and myMouse.isPressedIn(unit['unit']) and unit['can_move']:#len(unit['neighbours']) == 1:# and not release:
            target = unit
    ...
    if myMouse.isPressedIn(gallery):
        ...
        if not mouse_down_detected:
            gallery_number += 1
            frequency = save_to_gallery(positions, subject, gallery_number, date)
            points = update_points(points, frequency)
            drw_units()
            drw_gallery()
            win.flip()
    ...
    feedback(points,round(clock.getTime()))
    win.flip()
msg(bye)